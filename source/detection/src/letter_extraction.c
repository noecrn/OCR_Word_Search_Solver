  ...
  for (int row = 0; row < num_rows; row++) {
    for (int col = 0; col < num_cols; col++) {
      ...
      SDL_Surface *cell_surface =
          SDL_CreateRGBSurface(0, (int)cell_width, (int)cell_height, 32,
...
void split_word_into_images(SDL_Surface *surface, int letter_left,
  ...
  SDL_Surface *letter_surface =
      SDL_CreateRGBSurface(0, letter_width, letter_height, 32, 0x00FF0000,
  ...
  snprintf(filename, sizeof(filename), "%s/word_%d_letter_%d.png",
           words_directory, current_word, current_letter);
