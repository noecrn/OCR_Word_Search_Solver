...
int check_nearby_pixels(SDL_Surface *surface, int x, int y, int Height,
                        int Width) {
...
Component count_pixels(SDL_Surface *surface, int *visited, int Height,
                       int Width, int start_x, int start_y) {
...
int detect_grid(SDL_Surface *surface, int *biggest_component, int flag) {
  ...
  *biggest_component =
      largest.size; // Store the size by dereferencing the pointer
  if (largest.pixels) {
    free(largest.pixels);
  }
  free(visited);
