...
int count_words (
	...
	for (int y = list_top; y <= list_bottom; y++) {
		int black_pixel_count = count_black_pixels_on_a_line(surface, list_left, list_right, y, -1);
...
void letters_extraction (
	...
	for (int x = list_left; x <= list_right; x++) {
		int black_pixel_count = count_black_pixels_on_a_line(surface, list_top, list_bottom, -1, x);
		...
		if (black_pixel_count > white_threshold) { // Starting a new letter
			...
			if (is_black) {
				...
				split_word_into_images(surface, letter_left_bound, letter_right_bound, list_top, list_bottom, current_word, current_letter);
				current_letter++;
			}
		}
	}
